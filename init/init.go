...
loop:
	...
	if r.Error != nil {
		if strings.Contains(r.Error.Error(), "doesn't exist") {
			...
			util.Loggrs.Info("[init]. metadb init completed.")
			util.Loggrs.Info("[init]. please set up the account password and connection address in the configuration table.")
			os.Exit(-1)
		}
		fmt.Println(r.Error.Error())
		os.Exit(-1)
