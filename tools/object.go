...
func Size(s string) float64 {
	...
	if strings.Contains(s, "mb") || strings.Contains(s, "m") {
		return float * 1024 * 1024
	}
	if strings.Contains(s, "gb") || strings.Contains(s, "g") {
		return float * 1024 * 1024 * 1024
	}
	if strings.Contains(s, "tb") || strings.Contains(s, "t") {
...
func Version(app string, db *gorm.DB) float64 {
	if val, ok := util.Lastcache.Get(app + "version"); ok {
		return val.(float64)
	}
	...
	util.Lastcache.Set(app+"version", version, cache.DefaultExpiration)
	util.Loggrs.Infof("vers actual:%v", version)
	return version
}
...
ORDER BY query_concurrency_per_second DESC`, util.Read.Schema.Auditops)).Scan(&concurrency)
	...
	for _, item := range combineMetrics(cpu, mem, concurrency) {
		...
		val := fmt.Sprintf(`cpu:%v memory:%v concurrency:%v`, cpu_usage_percentage, max_mem_mb, query_concurrency_per_second)
		cachelimit.Set(app+item["user"].(string)+"operational", val, cache.DefaultExpiration)
		...
		cachelimit.Set(app+item["user"].(string)+"operationalmsg", msg, cache.DefaultExpiration)
	}
	util.Loggrs.Info("actual Operational ", app)
}
...
// IsUniform
// 判断数组中是否所有元素都一致
func IsUniform(arr []string, target string) bool {
	for _, v := range arr {
		if v != target {
			return false
		}
	}
	return len(arr) > 0 // 空数组返回false
}
...
// RmElement 数组中删除某个元素
func RmElement(slice []string, s string) []string {
	var arr []string
