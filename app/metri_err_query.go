...
func initMeta(app string) util.ConnectParms {
	...
	for _, m := range util.MetaLink {
		if m["app"].(string) == app {
			var managerAccessKey, managerSecretKey, managerHost, muser, mpass, mhost string
			if m["manager_access_key"] != nil {
				managerAccessKey = m["manager_access_key"].(string)
			}
			if m["manager_secret_key"] != nil {
				managerSecretKey = m["manager_secret_key"].(string)
			}
			if m["address"] != nil {
				managerHost = m["address"].(string)
			}
			if m["meta_user"] != nil {
...
func metainfo(item util.ConnectParms, begin, end string) []map[string]interface{} {
	if item.MetaUser == "" || item.MetaPass == "" || item.MetaHost == "" {
		...
		util.Loggrs.Warn(string(marshal))
		return nil
	}
...
// 收集报错信息
func showErrors(app string, db *gorm.DB, begin, end string) (errs []showErr) {
	avg := initMeta(app)
	// 从mysql元数据中分析数据
	...
	if len(m) >= 1 {
		for _, m2 := range m {
			var tag []string
			cpuSecond := int(m2["cpuCostNs"].(int64) / 1000000000)
			if cpuSecond > 0 {
				tag = append(tag, fmt.Sprintf(`cpu:[<span style="color: red;">%v</span>]`, tools.GetHour(cpuSecond)))
			}
			if m2["memCostBytes"].(int64) > 0 {
				tag = append(tag, fmt.Sprintf(`memory:[<span style="color: red;">%v</span>]`, tools.FormatBytes(m2["memCostBytes"].(int64))))
			}
			if m2["scanRows"].(int64) > 0 {
				tag = append(tag, fmt.Sprintf(`rows:[<span style="color: red;">%v</span>]`, m2["scanRows"].(int64)))
			}
			if m2["scanBytes"].(int64) > 0 {
				tag = append(tag, fmt.Sprintf(`bytes:[<span style="color: red;">%v</span>]`, tools.FormatBytes(m2["scanBytes"].(int64))))
			}

			queryId := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('%s:%d/getstmtid?app=%s&id=%s&begintime=%s&endtime=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
			...
			errs = append(errs, showErr{
				...
				User:      fmt.Sprintf("%s", m2["user"].(string)),
				Queryid:   queryId,
	...
	case version >= 3.3 && version != 3.39:
		util.Loggrs.Info(fmt.Sprintf("scan1> source:%d", len(m)))
		msgs := scan1(app, avg, m)
		errs = append(errs, msgs...)
	case version == 3.39:
		util.Loggrs.Info(fmt.Sprintf("scan2> source:%d", len(m)))
		msgs := scan2(app, avg, m)
		errs = append(errs, msgs...)
	default:
		util.Loggrs.Info(fmt.Sprintf("scan0> source:%d", len(m)))
...
func scan0(app string, avg util.ConnectParms, m []map[string]interface{}) []showErr {
	...
	for _, m2 := range m {
		...
		go func(m2 map[string]interface{}) {
			...
			queryId := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('%s:%d/getstmtid?app=%s&id=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
