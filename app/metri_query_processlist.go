...
func (engine *threadMap) strfunc(db *gorm.DB, app string, felist []string, mfe []map[string]string) dataItem {
	...
	for _, ip := range felist {
		...
		if tools.Version(app, db) >= 3.5 {
			break
		}
	}

	//进行第一次筛选
	var userQueryProcess, userSleepProcess []util.Process
	var userQueryLimit, userSleepLimit []string
	for _, api := range resultmsg {
		for _, process := range api.Prolist {
			// 判断睡眠连接
			switch process.Command {
			case "Sleep":
				userSleepLimit = append(userSleepLimit, process.User)
				userSleepProcess = append(userSleepProcess, util.Process{
					Id:        process.Id,
					User:      process.User,
					Host:      process.Host,
			...
			case "Query":
				userQueryLimit = append(userQueryLimit, process.User)
				userQueryProcess = append(userQueryProcess, util.Process{
					Id:        process.Id,
					User:      process.User,
					Host:      process.Host,
					Cluster:   process.User,
	...
	//统计query
	sumTag1 := countUsers(userQueryProcess)
	for _, user := range tools.RmDuplicaSlice(userQueryLimit) {
		limit := sumTag1[user]
		var concurrencyLimit []string
		if limit != 0 {
			concurrencyLimit = append(concurrencyLimit, fmt.Sprintf("在线会话:%d", limit))
			//统计sleep
			sumTag2 := countUsers(userSleepProcess)
			limit := sumTag2[user]
			if limit != 0 {
				concurrencyLimit = append(concurrencyLimit, fmt.Sprintf("挂起会话:%d", limit))
			}
			cachelimit.Set(user+"concurrency_limit", strings.Join(concurrencyLimit, "\n"), cache.DefaultExpiration)
		}
	}
	second30cache.Set(app+"sleepConnect", userSleepProcess, cache.DefaultExpiration)
	for _, m := range resultmsg {
		for _, process := range m.Prolist {
			...
			labela := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('%s:%d/getstmtid?app=%s&id=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
			...
			var runItem util.RunQueries
			var globalItem util.GlobalQueries
			if val, ok := second20cache.Get(fmt.Sprintf("%s.%s", app, process.Id)); ok {
				globalItem = val.(util.GlobalQueries)
				if val, ok := second20cache.Get(fmt.Sprintf("%s.%s", app, globalItem.QueryId)); ok {
					runItem = val.(util.RunQueries)
				}
			}
			//分析并发数
			var limitStr string
			...
			userurl := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('%s:%d/ugapi?app=%s&user=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
				util.H.Ip, util.Read.Server.Port, app, process.User, process.User)

			var feip string
			if val, ok := apicache.Get(app + process.User + "resource_group"); ok {
				feip = fmt.Sprintf(`%v
计算权重:%v
计算核心:%v
计算内存:%v
...
内存限制:%v
并发限制:%v`, limitStr,
					val.(map[string]interface{})["cpu_weight"].(interface{}),
					val.(map[string]interface{})["exclusive_cpu_cores"].(interface{}),
					val.(map[string]interface{})["mem_limit"].(interface{}),
					val.(map[string]interface{})["big_query_cpu_second_limit"].(interface{}),
					val.(map[string]interface{})["big_query_scan_rows_limit"].(interface{}),
					val.(map[string]interface{})["big_query_mem_limit"].(interface{}),
					val.(map[string]interface{})["concurrency_limit"].(interface{}),
				)
			}
			...
			result = append(result, queryResult{
				...
				Command:   fmt.Sprintf(`<button id="kill-connectid" style="border: none; outline: none;background: transparent;" class="bi bi-power text-danger" data-connectid="%s"></button>`, process.Id),
				Time:      edtime,
				State:     process.State,
				IsPending: ok,
				Info:      process.Info,
				//Warehouse:   fmt.Sprintf(`<button id="kill-disconnect" class="btn btn-light btn-sm ms-2" data-disconnect="%s">⛔</button>`, process.User),
				Warehouse:   fmt.Sprintf(`<button id="kill-disconnect" style="border: none; outline: none;background: transparent;" class="bi bi-slash-circle text-danger" data-disconnect="%s"></button>`, process.User),
				IsSlow:      isslow,
				...
				ScanBytes:   globalItem.ScanBytes,
				ScanRows:    globalItem.ScanRows,
				MemoryUsage: globalItem.MemoryUsage,
				CPUTime:     globalItem.CPUTime,
				Fragments:   runItem.Fragments,
				Dop:         runItem.Dop,
			})
...
func (engine *threadMap) httpapi(db *gorm.DB, app string, felist []string) dataItem {
	...
	for _, api := range resultmsg {
		for _, datum := range api.Data {
			...
			case "Sleep":
				userSleepLimit = append(userSleepLimit, datum.User)
				userSleepProcess = append(userSleepProcess, util.Process{
					Id:        datum.ID,
					User:      datum.User,
					Host:      datum.Host,
			...
			case "Query":
				...
				userQueryLimit = append(userQueryLimit, datum.User)
				userQueryProcess = append(userQueryProcess, util.Process{
					Id:        datum.ID,
					User:      datum.User,
					Host:      datum.Host,
	...
	for _, api := range resultmsg {
		for _, datum := range api.Data {
			...
			var runItem util.RunQueries
			var globalItem util.GlobalQueries
			if val, ok := second20cache.Get(fmt.Sprintf("%s.%s", app, datum.ID)); ok {
				globalItem = val.(util.GlobalQueries)
				if val2, ok := second20cache.Get(fmt.Sprintf("%s.%s", app, globalItem.QueryId)); ok {
					runItem = val2.(util.RunQueries)
				}
			}
			...
			var feip string
			if val, ok := apicache.Get(app + datum.User + "resource_group"); ok {
				feip = fmt.Sprintf(`%v
计算权重:%v
计算核心:%v
计算内存:%v
...
并发限制:%v`, limitStr,
			...
			result = append(result, queryResult{
				...
				Clientuser:  clientname,
				Db:          datum.Db,
				Command:     fmt.Sprintf(`<button id="kill-connectid" style="border: none; outline: none;background: transparent;" class="bi bi-power text-danger" data-connectid="%s"></button>`, datum.ID),
				Time:        Time,
				State:       datum.State,
				IsPending:   isPending,
				Info:        datum.Info,
				Warehouse:   fmt.Sprintf(`<button id="kill-disconnect" style="border: none; outline: none;background: transparent;" class="bi bi-slash-circle text-danger" data-disconnect="%s"></button>`, datum.User),
				IsSlow:      isslow,
				IsCritical:  iscritical,
				Whitelist:   whitelist,
				Feip:        feip,
