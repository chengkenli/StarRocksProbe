...
func showstream2load(app string, db *gorm.DB, beginTime, endTime string) []showErr {
	...
	for _, item := range m {
		...
		queryId := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('http://%s:%d/getstreamloadid?app=%s&id=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
			util.H.Ip, util.Read.Server.Port, app, item["LOAD_ID"].(string), fmt.Sprintf("%s.%s", item["DB_NAME"].(string), item["TABLE_NAME"].(string)))
