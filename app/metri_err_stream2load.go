...
func showstream2load(app string, db *gorm.DB, beginTime, endTime string) []showErr {
	...
	for _, item := range m {
		...
		queryId := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('%s:%d/getstreamloadid?app=%s&id=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
