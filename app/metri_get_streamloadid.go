...
func (engine *threadMap) metriStreamloadId(c *gin.Context) {
	...
	r := db.Raw(fmt.Sprintf("select * from information_schema.stream_loads where LOAD_ID='%s'", jobid)).Scan(&m)
	if r.Error != nil {
	...
	uri := fmt.Sprintf("%s:%d/log/%s", util.H.Ip, util.Read.Server.Port, filename)
