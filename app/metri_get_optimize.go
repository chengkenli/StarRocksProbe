		...
		for _, c1 := range column {
			...
			result = append(result,
				util.TaskOptimize{
					...
					Timeout:       c1["Timeout"],
					Tag:           "COLUMN",
					Command:       fmt.Sprintf(`<button id="cancel-optimize" style="border: none; outline: none;background: transparent;" class="bi bi-power text-danger" data-optimize="%s"></button>`, fmt.Sprintf("COLUMN:%s.%v", database, c1["TableName"])),
				})
		}
		...
		for _, c1 := range rollup {
			...
			result = append(result,
				util.TaskOptimize{
					...
					Timeout:       c1["Timeout"],
					Tag:           "ROLLUP",
					Command:       fmt.Sprintf(`<button id="cancel-optimize" style="border: none; outline: none;background: transparent;" class="bi bi-power text-danger" data-optimize="%s"></button>`, fmt.Sprintf("ROLLUP:%s.%v", database, c1["TableName"])),
				})
		}
		/*
		...
		for _, c1 := range optimize {
			...
			result = append(result,
				util.TaskOptimize{
					...
					Timeout:       c1["Timeout"],
					Tag:           "OPTIMIZE",
					Command:       fmt.Sprintf(`<button id="cancel-optimize" style="border: none; outline: none;background: transparent;" class="bi bi-power text-danger" data-optimize="%s"></button>`, fmt.Sprintf("OPTIMIZE:%s.%v", database, c1["TableName"])),
				})
		}
	}
