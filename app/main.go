...
func App() {
	...
	r.GET("/getsecurity", engine.refreshSecure)
	...
	admin.Use(tokenAuth()) // 应用管理员认证中间件
	{
		...
		admin.GET("/numrun", engine.numRun)
		...
		admin.GET("/refresh_security", engine.refreshSecure)
		admin.POST("/dropsecurity", engine.dropsecurity)
		admin.GET("/metadata", engine.metadata)
		admin.GET("/refresh_metadata", engine.refreshMetadata)
		admin.GET("/replica", engine.replica)
		admin.GET("/refresh_replica", engine.refreshReplica)
		admin.GET("/report_path", engine.report)
		admin.GET("/memory_usage", engine.memoryUsage)
		admin.GET("/explain_usage", engine.explainUsage)
	}
	port := fmt.Sprintf(":%d", util.Read.Server.Port)
	var err error
	if util.Read.Server.Ssl.Crt != "" && util.Read.Server.Ssl.Key != "" {
		err = r.RunTLS(port, util.Read.Server.Ssl.Crt, util.Read.Server.Ssl.Key)
	} else {
		err = r.Run(port)
	}
	if err != nil {
