...
func countProces(processes []util.Process) []util.CountProcess {
	...
	for user, count := range userCountMap {
		result = append(result, util.CountProcess{
			User:    user,
			Count:   count,
			Command: fmt.Sprintf(`<button id="kill-disconnect" style="border: none; outline: none;background: transparent;" class="bi bi-slash-circle text-danger" data-disconnect="%s"></button>`, user),
		})
	}
	// 按 Count 降序排序
