...
var second30cache = cache.New(30*time.Second, 60*time.Second)
...
func (engine *threadMap) _init2() {
	...
	go func() {
		if util.ClientIPDec == nil {
			db, err := engine.getmapConnect(_app())
			if err != nil {
				util.Loggrs.Error(err.Error())
				util.Loggrs.Fatalf(fmt.Sprintf("校验失败！配置文件中的主集群 ipapp:%s 在元数据表%s 中匹配不到连接信息，请确保集群名称一致！", _app(), util.Read.Metadb.Base))
				return
			}
			getsign(db)
		}
