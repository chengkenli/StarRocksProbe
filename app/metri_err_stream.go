...
func restyStream(restys *resty.Client, appid, leader string) []showErr {
	...
	for _, node := range table {
		...
		for _, node := range table {
			...
			//	util.Loggrs.Error(err.Error())
			//}
			label := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('%s:%d/getstream?app=%s&id=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
