...
func (engine *threadMap) showSubmit(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"data": sortByStartTimeDesc3(item.data), "running": item.running, "pending": item.pending, "merged": item.merged, "total": item.running + item.pending + item.merged, "threshold": getfc2key(appid, "max_running_txn_num_per_db", db)})
}
...
func showSubmit(app string, db *gorm.DB, beginTime, endTime string) submitData {
	...
	for _, m2 := range m {
		...
		queryId := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('%s:%d/getsubmit?app=%s&id=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
		...
		var tag string
		if strings.Contains(m2.TASK_NAME, "mv-") {
			tag = "物化视图"
		} else if strings.Contains(m2.TASK_NAME, "setbucket") {
			tag = "优化分桶"
		} else {
			tag = "异步任务"
		}
		...
		infos = append(infos,
			util.TaskRuns{
				...
				Command:     fmt.Sprintf(`<button id="cancel-task" style="border: none; outline: none;background: transparent;" class="bi bi-power text-danger" data-task="%s"></button>`, m2.TASK_NAME),
				Tag:         tag,
			})
	}
