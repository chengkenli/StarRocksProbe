...
func (engine *threadMap) disconnect(c *gin.Context) {
	util.Loggrs.Infof("%s disconnect", c.ClientIP())
	...
	util.Loggrs.Info("kill-disconnect:", strings.Join(ids.User, ","))
		...
		go func() {
			...
			for _, item := range dbresult {
				...
				go func() {
					...
					if !tools.StrInSlice(item["User"].(string), ids.User) {
						return
					}
					...
					util.Loggrs.Info(appid, ">", "处理请求成功，kill disconnect:", item["Id"].(int64), " ", item["User"].(string))
