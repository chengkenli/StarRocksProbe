...
func (engine *threadMap) showBroker(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"data": sortByStartTimeDesc2(item.data), "loading": item.loading, "pending": item.pending, "queueing": item.queueing, "prepared": item.prepared, "total": item.total, "threshold": getfc2key(appid, "max_broker_load_job_concurrency", db)})
}
	...
	for _, m2 := range m {
		...
		go func(m2 map[string]interface{}) {
			...
			for _, load := range tbLoads {
				...
				queryId := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('%s:%d/getbrokid?app=%s&id=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
				...
				infos = append(infos, util.BrokerMsg{
					...
					Backends:    backends,
					ScanInfo:    scanInfo,
					Command:     fmt.Sprintf(`<button id="cancel-label" style="border: none; outline: none;background: transparent;" class="bi bi-power text-danger" data-label="%s"></button>`, fmt.Sprintf("%s.%s", database, load.Label)),
		...
		go func(m2 map[string]interface{}) {
			...
			for _, load := range tbLoads {
				...
				errs = append(errs, showErr{
					...
					Errinfo:   load["TrackingSQL"].(string),
