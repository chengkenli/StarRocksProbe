...
loop:
	for {
		...
		case <-ticker.C:
			...
			once.Do(func() {
				util.Loggrs.Info("等待获取statistic元数据！")
			})
		}
	}
	...
	go func() {
		defer wg.Done()
		// 配置巡检【完成】
		if util.Read.Server.Sshuser == "" || felogpath == "" || belogpath == "" {
			scoreConf = 100
			diffsumConf = 0
			tools.WriteFile(fmt.Sprintf("%s/%s_log.html", insdir, app), "[免密账号] or [fe路径] or [be路径]为空，不支持巡检配置！")
			return
		}
	...
	go func() {
		...
		if util.Read.Server.Sshuser == "" {
			scoreLog = 100
			diffsumLog = 0
			tools.WriteFile(fmt.Sprintf("%s/%s_log.html", insdir, app), "[免密账号]为空，不支持巡检日志！")
			return
		}
