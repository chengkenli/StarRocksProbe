...
func loginsp(diffSum int, felogpath, belogpath string, fs, bs []map[string]interface{}) ([]string, int) {
	...
	var button, tabButton, fecontent, becontent, felog, belog, body []string
	// FE日志巡检
	for i, f := range fs {
		// 设置日志按钮
		if i == 0 {
	...
	// 设置整个标题的tab
	tabButton = append(tabButton, fmt.Sprintf(`<div class="tabs">%s</div>`, strings.Join(button, "\n")))
	// 设置内容
